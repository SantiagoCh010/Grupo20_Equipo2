...
def conectar():
    try: 
        conn = sqlite3.connect('bd/datos.db')
        return conn
    except Error as err:
        print("Ocurrio un error al establecer la conexión: " + str(err))
        return None
...
def ejecutar_insert(_sql, lista_parametros):
    try:
        ...
        if conn:
            ...
            print("No se pudo ver establecer la conexión.  Ver errores previos.")
            return -1
    except Error as err:
...
def ejecutar_select(_sql, lista_parametros):
    try:
        ...
        if conn:
            ...
            print("No se puedo establecer la conexión. Ver errores previos.")
            return None
    except Error as err:
        print("Ocurrió un error al intentar ejecutar el comando: " + _sql + " " + str(err))
        return None 
